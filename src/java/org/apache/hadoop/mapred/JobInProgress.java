...
public class JobInProgress {
  ...
  Map<Node, List<TaskInProgress>> createCache(
    ...
    it=_rackLastAssign.entrySet().iterator();
    while(it.hasNext()){
  ...
  //Add by RH for DegradedFirst at Oct 17th start
  //This function returns a degraded task.
  public synchronized Task obtainNewDegradedMapTask(TaskTrackerStatus tts,
                                                     int clusterSize, 
                                                     int numUniqueHosts)
  throws IOException {
    if (!tasksInited.get()) {
    ...
    Task result = maps[tip.getIdWithinJob()].getTaskToRun(tts.getTrackerName());
    if (result != null) {
